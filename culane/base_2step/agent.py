...
class Agent(nn.Module):
    ...
    def __init__(self):
        ...
        self.optimizer_gru = torch.optim.SGD(self.gru.parameters(), lr=0.00005, momentum=0.9)
    ...
    ## train
    #####################################################
    def train(self, inputs, target_lanes, target_h, epoch, agent, data_list, vp_gt):
        point_loss, gru_loss = self.train_point(inputs, target_lanes, target_h, epoch, data_list, vp_gt)
        return point_loss, gru_loss

    #####################################################
    ## compute loss function and optimize
    #####################################################
    def train_point(self, inputs, target_lanes, target_h, epoch, data_list, vp_gt):
        ...
        for i in range(real_batch_size):
            ...
            x, y = self.generate_result(confidence_, offset_, instance_, p.threshold_point)
            x, y = self.eliminate_fewer_points(x, y)
            left_pt, right_pt = util.ego_lane(x, y)
            # print("left----------------")
            left_pt = self.spline_lane(left_pt)
            # print("right----------------")
            right_pt = self.spline_lane(right_pt)
            ...
            # print('left:', ego_left_pts)
            # print('right:', ego_right_pts)
            # print("--------------------------------------------------------------")
        ...
        # print(ego_left_pts)
        # print(np.array(ego_left_pts).shape)
        # print("torch_left: ",torch.Tensor(ego_left_pts))
        # print("torch_right: ",torch.Tensor(ego_right_pts))
        if ego_left_pts != []:
            self.optimizer_gru.zero_grad() 
            vp_gt_used = torch.from_numpy(np.array(vp_gt_used, dtype='float32')).cuda()
            pred_vp = self.gru(torch.from_numpy(np.array(ego_left_pts, dtype='float32')).cuda(), torch.from_numpy(np.array(ego_right_pts, dtype='float32')).cuda())
            loss_gru = self.criterion_gru(pred_vp, vp_gt_used)
            loss_gru.backward()
            self.optimizer_gru.step()
            # print('gru loss:', loss_gru.item())
        ...
        if epoch>0 and self.current_epoch != epoch:
            ...
            if epoch == 1-trim:
                ...
                self.setup_optimizer()
        if loss_gru == 0:
            return lane_detection_loss, loss_gru
        else:
            return lane_detection_loss, loss_gru.item()
    #####################################################
    ## spline lane
    #####################################################
    ...
    ## predict VP in test
    #####################################################
    def predict_vp_test(self, vp_gt_used, ego_left_pts, ego_right_pts):
        vp_gt_used = torch.from_numpy(np.array(vp_gt_used, dtype='float32')).cuda()
        pred_vp = self.gru(torch.from_numpy(np.array(ego_left_pts, dtype='float32')).cuda(), torch.from_numpy(np.array(ego_right_pts, dtype='float32')).cuda())
        loss_gru = self.criterion_gru(pred_vp, vp_gt_used)
        # print('---------------------agent-------------------------')
        ...
        # print(vp_gt_used)
        # print('----------------------------------')
        return loss_gru, pred_vp.cpu().detach().numpy()
    ...
    def load_weights(self, epoch, loss, gru_loss):
        ...
        self.gru.load_state_dict(
            torch.load(self.p.model_path+str(epoch)+'_'+str(loss)+'_' + str(gru_loss) + '_'+'lane_detection_network_gru.pkl', map_location='cuda:0'), False
        )
    ...
    def save_model(self, epoch, loss, loss_gru):
        ...
        torch.save(
            self.gru.state_dict(),
            self.p.save_path+str(epoch)+'_'+str(loss)+'_'+str(loss_gru)+'_'+'lane_detection_network_gru.pkl'
        )
