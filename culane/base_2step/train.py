...
def Training():
    ...
    if p.model_path == "":
        ...
        lane_agent.load_weights(2091, "tensor(1.2316)", "0.01291594561189413") #17에폭
    ...
    for epoch in range(p.n_epoch):
        ...
        for inputs, target_lanes, target_h, test_image, data_list, vp_gt in loader.Generate(sampling_list):
            ...
            #training
            loss_p, loss_gru = lane_agent.train(inputs, target_lanes, target_h, epoch, lane_agent, data_list, vp_gt)
        ...
        if epoch%1 == 0:
            ...
            #     test.evaluation([0,0,0], loader, lane_agent, index = idx, name="test_result_"+str(epoch)+"_"+str(idx)+".json")
...
def testing(vp_gt, lane_agent, test_image, step, loss, loss_gru):
    ...
    _, _, ti, _ = test.test([0,0,0], vp_gt, lane_agent, np.array([test_image]))
