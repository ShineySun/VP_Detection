...
class Agent(nn.Module):
    ...
    def train_point(self, inputs, target_lanes, target_h, epoch, data_list, vp_gt):
        ...
        result, attentions, vp_info = self.predict_lanes(inputs, vp_gt)
        ...
        if epoch>0 and self.current_epoch != epoch:
            ...
            if epoch == 1-trim:
                ...
                self.setup_optimizer()
        if pred_vp is None:
            return lane_detection_loss, 0
        else:
            return lane_detection_loss, gru_loss.item()
    #####################################################
    ## spline lane
    #####################################################
    ...
    def predict_lanes_test(self, inputs, vp_gt):
        inputs = torch.from_numpy(inputs).float() 
        inputs = Variable(inputs).cuda()
        outputs, features, vp_info = self.lane_detection_network(inputs, vp_gt)
        return outputs, vp_info
