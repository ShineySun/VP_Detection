...
def Testing():
    ...
    if p.model_path == "":
        ...
        lane_agent = agent.Agent()
        lane_agent.load_weights(24, "tensor(0.8876)", "0.013993153348565102")
...
def evaluation(test_loss, loader, lane_agent, thresh = p.threshold_point, index= -1, name = None):
    ...
    for test_image, ratio_w, ratio_h, path, target_h, target_lanes, vp_gt in loader.Generate_Val():
        x, y, _, _ = test(test_loss, vp_gt, lane_agent, test_image, thresh, index= index)
        x_ = []
        y_ = []
        for i, j in zip(x, y):
...
def evaluation_test(test_loss, loader, lane_agent, thresh = p.threshold_point, index= -1, name = None):
    ...
    for test_image, ratio_w, ratio_h, path, target_h, target_lanes, vp_gt in loader.Generate_Test():
        x, y, _, used_idx_lst = test(test_loss, vp_gt, lane_agent, test_image, thresh, index=index)
        x_ = []
        y_ = []
        for i, j in zip(x, y):
            temp_x, temp_y = util.convert_to_original_size(i, j, ratio_w, ratio_h)
...
def test(test_loss, vp_gt, lane_agent, test_images, thresh = p.threshold_point, index= -1):
    ...
    for i in range(num_batch):
        ...
        # vp_gru test
        # print('-----------------------valid-------------------------------')
        left_pt, right_pt = util.ego_lane(in_x, in_y)
        # print(left_pt)
        # print('###################################3')
        # print(right_pt)
    ...
    if ego_left_pts != []:
        val_loss_gru, vp_pred = lane_agent.predict_vp_test(vp_gt_used, ego_left_pts, ego_right_pts)
...
##########################################
        ...
        test_loss[2] += len(used_idx_lst)
        
##########################################

        # print(x_pred, x_gt)
        # print(y_pred, y_gt)
        # print(np.sum(np.abs(x_pred-x_gt)), np.sum(np.abs(y_pred-y_gt)))
        # print(used_idx_lst)
        # print('gru loss_val:', val_loss_gru.item())
        # print('pred vp: ', vp_pred[:,0]*p.x_size)
        # print('pred vp: ', vp_pred[:,1]*p.y_size)
    # print('----------------------------------------------------------')




    return out_x, out_y,  out_images, used_idx_lst
